...
function formsubmit(command)
{
	...
	else
	{
		...
		if(!field.val())
		{ error.html(errormessage('Please Select Customer')); field.focus();return false;}
		
		passdata =  "switchtype=attachcard&customerreference=" + encodeURIComponent($('#customerlist').val()) 
			+ "&cardid=" + encodeURIComponent($('#scratchcardlist').val())
			+ "&description=" + encodeURIComponent($('#description').val())
			 + "&remarks=" + encodeURIComponent($('#remarks').val()) + "&dummy=" + Math.floor(Math.random()*100000000);
