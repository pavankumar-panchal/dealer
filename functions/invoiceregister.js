...
function getinvoicedetails(startlimit)
{
	...
	var queryString = "../ajax/invoiceregister.php";
		...
		{	
			...
			else
			{
				...
				if(response['errorcode'] == '1')
				{
					...
					$('#itemssummarydiv').html(response['servicegrid']);
				}
				else
				{
	...
	ajaxcall10 = $.ajax(
	{
		...
		{	
			...
			if(ajaxresponse['errorcode'] == '1')
			{
				...
				$('#itemssummarydiv').html(ajaxresponse['servicegrid']);
...
function filtertoexcel(command)
{
	...
	{
		error.html('');
		$('#submitform').attr("action", "../ajax/invoiceregistertoexcel.php?id=toexcel") ;
		$('#submitform').submit();
	}
}
