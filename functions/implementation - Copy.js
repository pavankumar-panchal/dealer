...
function formsubmit(command,type)
{
	...
	if(!field.val()) { alert("Confirm the selected invoice."); field.focus(); return false; }
...
function generateraffilegrid(startlimit)
{
	$('#form-error').html('');
	var form = $("#submitform");
	var passData = "switchtype=rafilegrid&lastslno=" + encodeURIComponent($('#customerlist').val()) + "&startlimit=" + encodeURIComponent(startlimit)+ "&dummy=" + Math.floor(Math.random()*10054300000);//alert(passData)
...
function approvevalidate()
{
	...
	if(result == false)
		return false;
	$("").colorbox({   height:"200px", inline:true, href:"#inline_example1" });
	$('#displaytab2').hide();
	 $('#displaytab3').hide();
	 $('#displaytab1').show();
}
...
function displayapproverejdiv(divtype)
{
	...
	if(divtype == 'approvetype')
	{
		...
		{
			$("").colorbox({ height:"250px",inline:true, href:"#inline_example1" });
		...
		{
			$("").colorbox({ height:"300px", inline:true, href:"#inline_example1" });
		...
		{	
			...
			else
			{
				...
				if(response1['errorcode'] == '1')
				{
					...
					var row = '<tr><td width="21%" ><strong>Selected Invoice: </strong></td><td width="20%"><input type="text" name="invoicedetails"  class="swiftselect-mandatory" disabled="disabled" id="invoicedetails" size="30"  value="'+ response1['invoicenumber']+'"  /></td><td width="59%"><div style="text-align:left"><a onclick="invoicedetailsdisplay(\'' + response1['customerreference'] + '\');"  class="r-text">Change &#8250;&#8250;</a></div></td></tr><tr ><td colspan="3">'+ response1['grid']+'</td> </tr>';
					$("#adddescriptionrows").append(row);
