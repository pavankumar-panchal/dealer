...
switch($switchtype)
{
	case 'save':
	{
		...
		$gst_no = $_POST['gst_no'];
		#new added fields for GST table
		$effective_from = $_POST['effective_from'];
		#new added fields for GST table ENDS
		...
		if($lastslno == "")
		{
			...
			$query = "Insert into inv_mas_customer(slno,customerid,businessname,address, place,pincode,district,region,category,type,stdcode,website,remarks,password,passwordchanged,disablelogin,createddate,createdby,corporateorder,currentdealer,fax,activecustomer,lastmodifieddate,lastmodifiedby,createdip,lastmodifiedip,branch,companyclosed, promotionalsms,promotionalemail,sez_enabled,tanno) values ('".$cusslno."','','".trim($businessname)."','".$address."','".$place."','".$pincode."','".$district."','".$regionid."','".$category."','".$type."','".$stdcode."','".$website."','".$remarks."','','N','no','".changedateformatwithtime($createddate)."','2','no','".$userid."','".$fax."','yes','".date('Y-m-d').' '.date('H:i:s')."','".$userid."','".$_SERVER['REMOTE_ADDR']."','".$_SERVER['REMOTE_ADDR']."','".$branchid."','".$companyclosed."','no','no','".$sez_enabled."','".$tanno."');";
			...
			if(!empty($gst_no)) 
			{
				...
				#added newly
				$insert_gst = "insert into customer_gstin_logs (gstin_id,customer_slno,effective_from,gst_no,created_by,updated_by,usertype,created_at,updated_at) values (NULL,$cusslno,'$effective_from'
				,'$gst_no','$userid','$userid','dealer_module',NOW(),NOW())";
				...
				$update_customer = "update inv_mas_customer set gst_no = ".$gstin_id_inserted." where slno = ".$cusslno;
		...
		elseif($lastslno <> "")
		{
			$query1 = "select editcustdata from inv_mas_dealer where slno = '".$userid."'";
			$fetch1 = runmysqlqueryfetch($query1);
			$editcustdata = $fetch1['editcustdata'];
			if($editcustdata == 'yes')
			{
				#new added fields for GST table
				$gstin_id_inserted = '';
			    if($gst_no!= "") {
					...
					$effective_from = (empty($effective_from)) ? '0000-00-00' : $effective_from;
            		$resultfetch_gst_new = runmysqlqueryfetch("select count(*) as gstnumbers from customer_gstin_logs where customer_slno = '$lastslno' and gst_no = '$gst_no' and effective_from <= '$effective_from' order by gstin_id desc limit 1");
    	            $gstnumbers = $resultfetch_gst_new['gstnumbers'];
    	            
    	            if($gstnumbers == 0) {
    	                
    	                $insert_gst = "insert into customer_gstin_logs (gstin_id,customer_slno,effective_from,gst_no,created_by,updated_by,usertype,created_at,updated_at) values (NULL,$lastslno,'$effective_from','$gst_no','$userid','$userid','dealer_module',NOW(),NOW())";
    			        $result_gst_new = runmysqlquery($insert_gst);
    	            }
					else
    	            {
    	            	$getquery = "select effective_from  from customer_gstin_logs where customer_slno = '$lastslno' and gst_no = '$gst_no';";
    	            	$getfetch = runmysqlqueryfetch($getquery);
						if($getfetch['effective_from'] == '0000-00-00')
						{
							$datequery = "update customer_gstin_logs set effective_from = '".$effective_from."' where customer_slno = '$lastslno' and gst_no = '$gst_no';";
							$dateresult = runmysqlquery($datequery);
						}
    	            }
				...
				$query3 = "Insert into inv_logs_save(customerid,businessname,address, place,pincode,district,region,category,type,stdcode,website,remarks,disablelogin,corporateorder ,currentdealer,fax,branch,companyclosed,date,time,system,isdealer,contactdetails,oldbusinessname,tanno,panno) values ('".$lastslno."','".$e_businessname."','".$e_address."','".$e_place."','".$e_pincode."','".$e_district."','".$e_region."','".$e_category."','".$e_type."','".$e_stdcode."','".$e_website."','".$e_remarks."','".$e_disablelogin."','".$corporateorder."','".$currentdealer."','".$fax."','".$branch."','".$companyclosed."','".datetimelocal('Y-m-d')."','".datetimelocal('H:i:s')."','".$_SERVER['REMOTE_ADDR']."','".$e_isdealer."','".$oldcontactarray."','".$e_oldbusinessname."','".$e_tanno."','".$e_panno."');";
				$result = runmysqlquery($query3);

				if($e_gstno == "" && $gstin_id_inserted!= "")
				{
					$addgst = 'gst_no='.$gstin_id_inserted.',';
				}
				
			    $query1 = "UPDATE inv_mas_customer SET businessname = '".trim($businessname)."',address = '".$address."',
			    place = '".$place."',pincode = '".$pincode."',district = '".$district."',category = '".$category."',type = '".$type."',
			    stdcode = '".$stdcode."',website = '".$website."',remarks = '".$remarks."',fax ='".$fax."',lastmodifieddate ='".date('Y-m-d').' '.date('H:i:s')."',lastmodifiedby ='".$userid."',
			    lastmodifiedip = '".$_SERVER['REMOTE_ADDR']."' ,companyclosed = '".$companyclosed."' ,
			    ".$addgst."sez_enabled = '".$sez_enabled."',tanno = '".$tanno."' WHERE slno = '".$lastslno."'";
				//echo $query1;
				//exit();
				...
				for($j=0;$j<count($contactressplit);$j++)
				{
					...
					if($slno <> '')
					{
						$query21 = "UPDATE inv_contactdetails SET contactperson = '".$contactperson."',phone = '".$phonevalue."',cell = '".$cellvalue."',
						emailid = '".$emailidvalue."',selectiontype = '".$selectiontype."' WHERE slno = '".$slno."'";
					...
					else
					{
						...
						values  
						('".$lastslno."','".$selectiontype."','".$contactperson."','".$phonevalue."','".$cellvalue."','".$emailidvalue."');";
						$result = runmysqlquery($query21);
					}
			...
			else
			{
				...
				else
				{
					...
					//$query_customer = "UPDATE inv_mas_customer SET gst_no = '".$gst_no."',sez_enabled = '".$sez_enabled."' WHERE slno = '".$lastslno."'";
					//$query_customer = "UPDATE inv_mas_customer SET gst_no = '".$gst_no."' WHERE slno = '".$lastslno."'";
					//$result1_customer = runmysqlquery($query_customer);
	...
	case 'forcesurrenderdetails':
	{
		...
		switch($switchforce)
		{
			...
			{				
				...
				$query2="select HDDID, ETHID,regcode,surrenderstatus from inv_customerproduct where slno = '".$refslno."'";
				$fetchresult2 = runmysqlqueryfetch($query2);
				$HDDID = $fetchresult2['HDDID'];
				$ETHID = $fetchresult2['ETHID'];
				$regcode = $fetchresult2['regcode'];
				$surrenderstatus = $fetchresult2['surrenderstatus'];
				if(($HDDID <> '' || $HDDID <> NULL) || ($ETHID <> '' || $ETHID <> NULL))
					$pinstatus = 'Active';
				elseif(!empty($regcode) && $surrenderstatus == 'no')
					$pinstatus = 'Active';
				else
					$pinstatus = 'Surrender';
			...
			case 'forcesurrender':
			{
				...
				inv_customerproduct.HDDID,inv_customerproduct.ETHID,inv_customerproduct.COMPUTERNAME,regcode,surrenderstatus,
				...
				$surrenderstatus = $fetchresult['surrenderstatus'];
				$regcode = $fetchresult['regcode'];
				...
				if($cardidcount <> 0)
				{
					if($HDDID!= '' || $ETHID!= '')
					{
						...
						$query2 = "UPDATE inv_customerproduct SET HDDID = '',ETHID = '',COMPUTERNAME = '',COMPUTERIP = '',CREATEDBY = '',reregistration='yes' WHERE cardid = '".$cardid."'";
						$result2 = runmysqlquery($query2);
					...
					else if(!empty($regcode) && $surrenderstatus == 'no')
					{
						$query = "Insert into inv_surrenderproduct(refslno,REGDATE,HDDID,ETHID, REGTYPE,COMPUTERNAME,COMPUTERIP,CREATEDBY,surrendertime,networkip,systemip,forces,forceremarks,userref) 
						values ('".$refslno."','".$REGDATE."','','','1','".$COMPUTERNAME."','".$COMPUTERIP."','".$CREATEDBY."','".date('Y-m-d').' '.date('H:i:s')."','".$_SERVER['REMOTE_ADDR']."','".$_SERVER['REMOTE_ADDR']."','1','".$forceremarks."','".$userid."');";
						$result = runmysqlquery($query);
						...
						$query2 = "UPDATE inv_customerproduct SET surrenderstatus='yes',COMPUTERNAME = '',COMPUTERIP = '',CREATEDBY = '',reregistration='yes' WHERE cardid = '".$cardid."'";
						$result2 = runmysqlquery($query2);
						...
						$eventquery = "Insert into inv_logs_event(userid,system,eventtype,eventdatetime,remarks) 
						values('".$userid."','".$_SERVER['REMOTE_ADDR']."','273','".date('Y-m-d').' '.date('H:i:s')."','".$lastslno."')";
						$eventresult = runmysqlquery($eventquery);
						##Sending Mail##
						sendsurrendermail();
						$message = '3^Cardid: '.$cardid.' Got Surrender Now Customer can Register!!';
	...
	case 'surrenderhistory':
	{
		...
		$result1 = runmysqlquery($query1);			
		$counthistory = mysqli_num_rows($result1);
 ...
 where inv_mas_dealer.slno = '".$userid."';";
	...
	case 'customerdetailstoform':
	{
		...
		if(mysqli_num_rows($countresult) == 0)
		{
			...
			$query_gst = "select gst_no as new_gst_no,effective_from from customer_gstin_logs where customer_slno=". $lastslno." order by gstin_id desc limit 1";
			...
			if($count_gst > 0)
			{
				...
				$effective_from = $fetch_gst['effective_from'];
			}
			else
			{
			...
			$responsearray['effective_from'] = $effective_from;
			...
			//added on 24/sep/2019
            $responsearray['tanno'] = $fetch['tanno'];
		...
		else
		{
			...
			inv_customerreqpending.pincode, inv_customerreqpending.fax, inv_customerreqpending.stdcode, inv_customerreqpending.website, inv_customerreqpending.category, inv_customerreqpending.type,inv_mas_branch.branchname as branch,inv_customerreqpending.companyclosed,inv_customerreqpending.promotionalsms,inv_customerreqpending.promotionalemail,inv_mas_customer.gst_no,
			inv_customerreqpending.remarks, inv_mas_customer.currentdealer, inv_mas_customer.password, inv_mas_customer.passwordchanged, inv_mas_customer.sez_enabled, 
			inv_mas_customer.disablelogin, inv_mas_customer.corporateorder,inv_mas_customer.activecustomer,inv_customerreqpending.createddate, inv_mas_users.fullname,inv_mas_product.productname, inv_mas_district.districtname as districtname,inv_mas_state.statename as statename,inv_mas_state.state_gst_code as state_gst,inv_mas_customer.tanno FROM inv_customerreqpending LEFT JOIN inv_mas_customer on inv_mas_customer.slno = inv_customerreqpending.customerid
			LEFT JOIN inv_mas_product ON inv_mas_product.productcode = inv_mas_customer.firstproduct 
			LEFT JOIN inv_mas_users ON inv_mas_customer.createdby = inv_mas_users.slno 
...
on registrations.registrations = groups.group order by groups.group desc"; //echo($query2);exit;
			...
			$grid .= '</table>';
				...
				//$responsecusidarray['gst_no'] = $fetch['gst_no'];
				$responsecusidarray['state_gst'] = $fetch['state_gst'];
				$responsecusidarray['sez_enabled'] = $fetch['sez_enabled'];
				$responsecusidarray['grid'] = $grid;
				$responsecusidarray['pendingrequestmsg'] = '';
                $responsecusidarray['last_new_gst_no'] = $new_gst_no;
                $responsecusidarray['effective_from'] = $fetch['effective_from'];
...
inv_customerreqpending.pincode, inv_customerreqpending.fax, inv_customerreqpending.stdcode, inv_customerreqpending.website, inv_customerreqpending.category, inv_customerreqpending.type,inv_mas_branch.branchname as branch,inv_customerreqpending.companyclosed,inv_customerreqpending.promotionalsms,inv_customerreqpending.promotionalemail,inv_mas_customer.gst_no,
 inv_customerreqpending.remarks, inv_mas_customer.currentdealer, inv_mas_customer.password, inv_mas_customer.passwordchanged, inv_mas_customer.gst_no, inv_mas_customer.sez_enabled, 
inv_mas_customer.disablelogin, inv_mas_customer.corporateorder,inv_mas_customer.activecustomer,inv_customerreqpending.createddate, inv_mas_users.fullname,inv_mas_product.productname, inv_mas_district.districtname as districtname,inv_mas_state.statename as statename,inv_mas_state.state_gst_code as state_gst FROM inv_customerreqpending LEFT JOIN inv_mas_customer on inv_mas_customer.slno = inv_customerreqpending.customerid
LEFT JOIN inv_mas_product ON inv_mas_product.productcode = inv_mas_customer.firstproduct 
 LEFT JOIN inv_mas_users ON inv_mas_customer.createdby = inv_mas_users.slno 
left join inv_mas_district on inv_customerreqpending.district = inv_mas_district.districtcode 
left join inv_mas_branch on inv_mas_branch.slno = inv_mas_district.branchid
...
on registrations.registrations = groups.group order by groups.group desc"; //echo($query2);exit;
            ...
            $responsearray['last_new_gst_no'] = $new_gst_no;
            $responsearray['effective_from'] = $fetch['effective_from'];
			echo(json_encode($responsearray));
		}
	}
	break;
	case 'customerregistration':
	{
		...
		if($startlimit == 0)
		{
		$grid = '<table width="100%" cellpadding="3" cellspacing="0" class="table-border-grid " id="myTable" >';
		$grid .= '<tr class="tr-grid-header" id="headerr"><td nowrap = "nowrap" class="td-border-grid pointer" align="left" >Sl No </td><td nowrap = "nowrap" class="td-border-grid pointer" align="left" onclick="sortTable(1,this)">Product Name <img src="../images/both.png" height="10px" width="10px"></td><td nowrap = "nowrap" class="td-border-grid pointer" align="left" onclick="sortTable(2)">PIN Number <img src="../images/both.png" height="10px" width="10px"></td><td nowrap = "nowrap" class="td-border-grid pointer" align="left" onclick="sortTable(3)">Computer ID <img src="../images/both.png" height="10px" width="10px"></td><td nowrap = "nowrap" class="td-border-grid pointer" align="left" onclick="sortTable(4)">Soft Key <img src="../images/both.png" height="10px" width="10px"></td><td nowrap = "nowrap" class="td-border-grid pointer" align="left" onclick="sortTable(5)">Date <img src="../images/both.png" height="10px" width="10px"></td><td nowrap = "nowrap" class="td-border-grid pointer" align="left">Time</td><td nowrap = "nowrap" class="td-border-grid pointer" align="left" onclick="sortTable(7)">Generatd By <img src="../images/both.png" height="10px" width="10px"></td><td nowrap = "nowrap" class="td-border-grid pointer" align="left" onclick="sortTable(8)">Dealer <img src="../images/both.png" height="10px" width="10px"></td><td nowrap = "nowrap" class="td-border-grid pointer" align="left" onclick="sortTable(9)">Bill No <img src="../images/both.png" height="10px" width="10px"></td><td nowrap = "nowrap" class="td-border-grid pointer" align="left" onclick="sortTable(10)">Bill Amount <img src="../images/both.png" height="10px" width="10px"></td><td nowrap = "nowrap" class="td-border-grid" align="left">Details</td><td nowrap = "nowrap" class="td-border-grid pointer" align="left">Remarks</td></tr>';
		}
		...
		$responsearray['selectforce'] = $selectforce;
		...
		$responsearray2['selectforce'] = $selectforce;
...
$eventquery = "Insert into inv_logs_event(userid,system,eventtype,eventdatetime,remarks) values('".$userid."','".$_SERVER['REMOTE_ADDR']."','115','".date('Y-m-d').' '.date('H:i:s')."','".$customerid."')";
	...
	case 'newregcardlist':
	{
		...
		where attached = 'yes' and registered = 'no' and blocked = 'no' and inv_dealercard.purchasetype = 'new'
		and inv_mas_product.newproduct!= 1 and inv_dealercard.dealerid = '".$userid."'";
		$result = runmysqlquery($query);
		$count = 0;
		while($fetch = mysqli_fetch_array($result))
		{
	...
	case 'updationcardlist':
	{
		...
		left join inv_dealercard on inv_mas_scratchcard.cardid = inv_dealercard.cardid
		left join inv_mas_product on inv_dealercard.productcode = inv_mas_product.productcode
		where attached = 'yes' and registered = 'no' and blocked = 'no' and inv_dealercard.purchasetype = 'updation'
		and inv_mas_product.newproduct!= 1 and inv_dealercard.dealerid = '".$userid."'";
		$result = runmysqlquery($query);
		$count = 0;
		while($fetch = mysqli_fetch_array($result))
...
where inv_dealercard.cardid = '".$cardlastslno."' and inv_mas_dealer.slno = '".$userid."'";
	...
	case 'matrixinvoicedetailsgrid':
	{
		$startlimit = $_POST['startlimit'];
		$slno = $_POST['slno'];
		$showtype = $_POST['showtype'];
		$lastslno = $_POST['lastslno'];
		$resultcount = "select * from inv_matrixinvoicenumbers where right(customerid,5) = '".$lastslno."' order by createddate  desc;";
		$resultfetch = runmysqlquery($resultcount);
		$fetchresultcount = mysqli_num_rows($resultfetch);
		if($showtype == 'all')
		$limit = 100000;
		...
		$query = "select * from inv_matrixinvoicenumbers  where right(customerid,5) = '".$lastslno."'  order by createddate  desc LIMIT ".$startlimit.",".$limit.";";
		$result = runmysqlquery($query);
		if($startlimit == 0)
		{
			$grid = '<table width="100%" cellpadding="3" cellspacing="0" class="table-border-grid">';
			...
			<td nowrap = "nowrap" class="td-border-grid" align="left">Status</td><td nowrap = "nowrap" class="td-border-grid" align="left">Generated By</td><td nowrap = "nowrap" class="td-border-grid" align="left">Action</td></tr>';
		}
		...
		while($fetch = mysqli_fetch_array($result))
		{
			...
			$grid .= "<td nowrap='nowrap' class='td-border-grid' align='left'>".$fetch['status']."</td>";
			$grid .= "<td nowrap='nowrap' class='td-border-grid' align='left'>".$fetch['createdby']."</td>";
			$grid .= '<td nowrap="nowrap" class="td-border-grid" align="center"> <a onclick="viewmatrixinvoice(\''.$fetch['slno'].'\');" class="resendtext" style = "cursor:pointer"> View >></a> </td>';
			$grid .= "</tr>";
		}
		$grid .= "</table>";
		...
		$linkgrid .= '<table><tr><td class="resendtext"><div align ="left" style="padding-right:10px"><a onclick ="getmorematrixinvoicedetails(\''.$lastslno.'\',\''.$startlimit.'\',\''.$slno.'\',\'more\');" style="cursor:pointer">Show More Records >></a>&nbsp;&nbsp;&nbsp;<a onclick ="getmorematrixinvoicedetails(\''.$lastslno.'\',\''.$startlimit.'\',\''.$slno.'\',\'all\');" class ="resendtext1" style="cursor:pointer"><font color= "#000000">(Show All Records)</font></a></div></td></tr></table>';
...
function surrender($PIN,$lastslno)
{
	...
	$checkresult = runmysqlquery($query08);
	$checksurrendercount = mysqli_num_rows($checkresult);
