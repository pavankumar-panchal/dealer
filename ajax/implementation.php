...
switch($switchtype)
{
	...
	case 'getcustomercount':
	{
		...
		$query = "select inv_mas_dealer.relyonexecutive, inv_mas_dealer.district, inv_mas_district.statecode, inv_mas_dealer.branch , inv_mas_dealer.region,inv_mas_dealer.maindealers,inv_mas_dealer.dealerhead from inv_mas_dealer left join inv_mas_district on inv_mas_dealer.district = inv_mas_district.districtcode where inv_mas_dealer.slno = '".$userid."';";
		...
		// 	OR inv_mas_customer.currentdealer = '".$userid."' or branch = '".$branch."' order by businessname;";
		// 	$result = runmysqlquery($query);
		// }		
	...
	case 'generatecustomerlist':
	{
		...
		// if($maindealers == 'yes')
		// {
		// 	$query = "select  distinct slno as slno, businessname as businessname from inv_mas_customer where 
		// 	inv_mas_customer.currentdealer IN (select slno from inv_mas_dealer where dealerhead = '".$userid."') 
		// 	OR inv_mas_customer.currentdealer = '".$userid."' or branch = '".$branch."' order by businessname LIMIT ".$startindex.",".$limit.";";
		// 	$result = runmysqlquery($query);
		// }
		#Ends		
		$customerarray = array();
	...
	case 'invoicedetails':
	{
		...
		if($fetch1['count'] > 0)
		{
			...
			if($fetch12['count'] > 0)
			{
				...
				// '862','864','262','261','269','266','267','268','237','238','877','878','879','880','882','883','884','001','885','886','887','888','889','890','891','892','800','931')
			...
			$query2 = "select servicename from inv_mas_service where enableservice = 'yes'";
			$result2 = runmysqlquery($query2);
			...
			if($count2 > 0)
			{
				...
				while($fetch3 = mysqli_fetch_array($result3))
				{
					...
					if(array_intersect($servicetype,(array)$servicename))
					{
	...
	case 'customervalidation':
	{
		...
		if($fetch1['count'] > 0)
		{
			...
			if($implementationstatus!= 'completed')
			{
				...
				where imp_implementation.customerreference = '".$lastslno."' and imp_implementation.slno = '".$impslno."'".$dealerpiece."";
			...
			else
			{
				...
				$responsearray1['errormsg'] = 'Create Implementation.';
				echo(json_encode($responsearray1));
			}
	...
	case 'save':
	{
		...
		if($lastslno == "")
		{
			...
			sendimprequestmail($refslno,$userid);
		...
		else
		{
			...
			// $e_implementationtype = $queryfetch['implementationtype'];
			// $e_impremarks = $queryfetch['impremarks'];
	...
	case 'implementationvalidate':
	{
		...
		where inv_dealercard.productcode in ('265','816','640','604','643','851','871','874','834','836','835','841','838',
		'843','844','850','869','872','875','870','873','876','861','863',
		'862','864','262','261','269','266','267','268','237','238','877','878','879','880','882','883','884','001','885','886','887','888','889','890','892','891','800','904','952')
	...
	case 'searchbycustomerid':
	{
		...
		if($maindealers == 'yes')
		{
			$dealerpiece .=  " and (inv_mas_customer.currentdealer IN (select slno from inv_mas_dealer where dealerhead = '".$userid."') OR inv_mas_customer.currentdealer = '".$userid."')";
		}		
	...
	case 'searchcustomerlist':
	{
		...
		// Customer Listing
		$query1 = "select inv_mas_dealer.relyonexecutive, inv_mas_dealer.district, inv_mas_district.statecode , inv_mas_dealer.branch , inv_mas_dealer.region,inv_mas_dealer.maindealers,inv_mas_dealer.dealerhead from inv_mas_dealer left join inv_mas_district on inv_mas_dealer.district = inv_mas_district.districtcode
		...
		if($maindealers == 'yes')
		{
		    $customerlistpiece .=  " and (inv_mas_customer.currentdealer IN (select slno from inv_mas_dealer where dealerhead = '".$userid."') 
			OR inv_mas_customer.currentdealer = '".$userid."')";
		}
		#Ends Added on 02 Feb		
	...
	case 'filtercustomerlist':
	{
		...
		if($status == "")
		{
			...
			left join imp_implementation on imp_implementation.customerreference = inv_mas_customer.slno
			left join inv_mas_district on inv_mas_customer.district = inv_mas_district.districtcode
			where inv_mas_customer.slno <> '99999999999' ".$customerlistpiece." ORDER BY businessname";
			$result = runmysqlquery($query);
		...
		else
		{
			...
			while($fetch0 = mysqli_fetch_array($result0))
			{
				...
				left join imp_implementation on imp_implementation.customerreference = inv_mas_customer.slno
				left join inv_mas_district on inv_mas_customer.district = inv_mas_district.districtcode
				where inv_mas_customer.slno = '".$fetch0['customerreference']."' and imp_implementation.slno = '".$fetch0['impslno']."' ".$statuspiece."";
				$result = runmysqlquery($query);
	...
	case 'invoicecfdetailstoform':
	{
		...
		if($fetch13['count'] == 0)
		{
			...
			$responsearray21['implementationtype'] = $fetch['implementationtype'];
			$responsearray21['impremarks'] = $fetch['impremarks'];
